...
export async function assinaturasRoutes(app) {
    ...
    app.get("/:token", async (req, reply) => {
        ...
        if (!request)
            return reply.status(404).send({ error: "Link de assinatura inválido." });
        if (request.status === "ASSINADO")
            return reply.status(410).send({ error: "Documento já assinado." });
    ...
    app.post("/:token", async (req, reply) => {
        ...
        if (request.status === "ASSINADO")
            return reply.status(410).send({ error: "Já assinado." });
        if (new Date() > request.expiresAt)
            return reply.status(410).send({ error: "Link expirado." });
        // Converte base64 para buffer
        const signatureBuffer = Buffer.from(assinaturaBase64.replace(/^data:image\/\w+;base64,/, ""), "base64");
