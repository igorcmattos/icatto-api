...
async function setPublicRead(fileId) {
    const drive = getDriveClient();
 ...
 */
export async function uploadFileToDrive({ contratoId, subpasta, fileName, mimeType, buffer }) {
    ...
    if (!contrato?.driveFolderId) {
        ...
        if (!updated?.driveFolderId)
            throw new Error("Não foi possível criar pasta no Drive.");
